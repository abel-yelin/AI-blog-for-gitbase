...
public class HandlePullRequestWebhookEndpoint
{
    private readonly OpenAIService _openAIService;

    public HandlePullRequestWebhookEndpoint(OpenAIService openAIService)
    {
        _openAIService = openAIService;
    }

    public async Task<IResult> HandleWebhook(int pullRequestNumber, AppConfig appConfig)
    {
    ...
    private async Task<EditContentResult> EditContentBasedOnComments(OpenAIConfig openAiConfig, string markdownContent, List<(string Path, int Line, string Comment)> commentsWithContext, string overallReviewComment)
    {
...
";
        ...
        string editedContent = await _openAIService.SendChatRequest(chatRequest.ToString());
        string commentResponse = await _openAIService.SendChatRequest(chatRequest.ToString());
